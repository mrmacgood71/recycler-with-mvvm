...
import it.macgood.rightrecyclerview.UserNotFoundException
import it.macgood.rightrecyclerview.task.SimpleTask
import it.macgood.rightrecyclerview.task.Task
import java.util.*
...
class UsersService {
    ...
    fun getUserById(userId: Long): Task<UserDetails> = SimpleTask {
        Thread.sleep(2000)
        val userIndex = users.firstOrNull { it.id == userId } ?: throw UserNotFoundException()

        return@SimpleTask UserDetails(
            userIndex,
    ...
    fun moveUser(user: User, moveBy: Int): Task<Unit> = SimpleTask {
        val oldIndex = users.indexOfFirst { it.id == user.id }
        if (oldIndex == -1) return@SimpleTask
        val newIndex = oldIndex + moveBy
        if (newIndex < 0 || newIndex >= users.size) return@SimpleTask
        val newUser = users[newIndex].copy()
        users = ArrayList(users)
    ...
    fun changeUser(user: User, name: String) {
        val index = users.indexOfFirst { it.id == user.id }
        val current = users[index].copy()
        current.name = name
        users = ArrayList(users)
        users[index] = current
        notifyChanges()
